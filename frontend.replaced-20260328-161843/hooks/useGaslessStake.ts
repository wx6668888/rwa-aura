...
import { getBrowserRelayerBase } from '@/lib/client-api-base';

/** 中继已广播 tx 但可能尚未打包；前端不应因短时拿不到 receipt 判失败 */
export type GaslessRelayerResult = { txHash: string; pending: boolean };
...
export function useGaslessStake() {
  ...
  const gaslessStake = async (
    ...
    try {
      ...
      const usdtNonceRes = await fetch(`${getBrowserRelayerBase()}/api/usdt-nonce/${address}`);
      if (!usdtNonceRes.ok) {
      ...
      console.log('Fetching staking nonce (post-permit)...');
      const nonceRes = await fetch(`${getBrowserRelayerBase()}/api/nonce/${address}`);
      if (!nonceRes.ok) {
      ...
      const res = await fetch(`${getBrowserRelayerBase()}/api/meta-stake-permit`, {
        method: 'POST',
      ...
      if (!result.success) {
        if (result.code === 'RELAYER_INSUFFICIENT_BNB' && result.relayerAddress) {
          throw new Error(`${result.error}（中继地址：${result.relayerAddress}）`);
        }
        throw new Error(result.error || 'Relayer rejected request');
      }
      
      console.log('Gasless stake successful!', result.txHash);
      return { txHash: result.txHash, pending: Boolean(result.pending) };
    } catch (error) {
  ...
  const gaslessStakeRWA = async (
    ...
    try {
      ...
      const rwaPermitNonceRes = await fetch(`${getBrowserRelayerBase()}/api/rwa-nonce/${address}`);
      ...
      // 质押 EIP-712 使用 Staking.nonces(user)；在 Permit 之后立刻再取一次，减少与其它 tab/请求竞态
      console.log('Fetching staking nonce (post-permit)...');
      const nonceRes = await fetch(`${getBrowserRelayerBase()}/api/nonce/${address}`);
      if (!nonceRes.ok) {
      ...
      // 3. 发送到中继
      console.log('Sending to relayer...');
      const res = await fetch(`${getBrowserRelayerBase()}/api/meta-stake-rwa-permit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user: address,
