...
export function PanelRwaYield({ onMobileBack, data }: Props) {
  ...
  // ËÆ°ÁÆó‰∏ãÊ¨°ÂèëÊîæÊó∂Èó¥ÔºàÊØèÂ§©08:00Âåó‰∫¨Êó∂Èó¥Ôºâ
  useEffect(() => {
  ...
  // ÂÆûÊó∂Êî∂ÁõäÂ¢ûÈïøÔºàÊØèÁßíÂ¢ûÂä†Ôºâ
  useEffect(() => {
    if (!hasYield) return
    
    setLiveYield(yieldAmount)
    
    // ÂÅáËÆæÊó•Êî∂ÁõäÁéá0.8%ÔºåÊØèÁßíÂ¢ûÈïø = ÊÄªË¥®Êäº * 0.008 / 86400
    // ËøôÈáåÁÆÄÂåñ‰∏∫ÊØèÁßíÂ¢ûÂä†‰∏Ä‰∏™ÂæàÂ∞èÁöÑÂÄº
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.error('ÊèêÂèñÂ§±Ë¥•:', err)
      
      let errorMessage = 'ÊèêÂèñÂ§±Ë¥•ÔºåËØ∑ÈáçËØï'
      if (err.message?.includes('User rejected') || err.message?.includes('User denied')) {
        errorMessage = 'ÊÇ®Â∑≤ÂèñÊ∂à‰∫§Êòì'
      } else if (err.message?.includes('insufficient funds')) {
        errorMessage = 'BNB ‰ΩôÈ¢ù‰∏çË∂≥ÔºåÊó†Ê≥ïÊîØ‰ªò Gas Ë¥πÁî®'
      } else if (err.message?.includes('execution reverted')) {
        errorMessage = 'ÂêàÁ∫¶ÊâßË°åÂ§±Ë¥•ÔºåËØ∑Ê£ÄÊü•ÊèêÂèñÈáëÈ¢ù'
      }
    ...
    <div className="flex flex-col h-full bg-[#0a0a0f]/40">
      ...
      <div className="flex items-center justify-between p-6 border-b border-green-500/10">
        ...
        >
          ...
          <span className="text-sm">∑µªÿ</span>
        </button>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">RWA  ’“Ê</h2>
            <p className="text-xs text-white/40 mt-0.5">√ø»’ 0.8% ÷ —∫Ω±¿¯</p>
          </div>
        </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        {hasYield ? (
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500/10 via-emerald-500/5 to-transparent border border-green-500/20 p-6">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold text-green-400">œ¬¥Œ∑¢∑≈µπº∆ ±</span>
                </div>
                <div className="flex items-center gap-4">
                  <div className="flex-1 grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-black/40 backdrop-blur-sm rounded-xl p-3 border border-green-500/20">
                      ...
                      <div className="text-xs text-white/40 mt-1">√Î</div>
                    </div>
                  </div>
                </div>
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500/10 via-green-500/5 to-transparent border border-emerald-500/20 p-6">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold text-emerald-400"> µ ±¥˝¡Ï»° ’“Ê</span>
                </div>
            ...
            <div className="bg-white/[0.02] backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              <div className="flex justify-between items-center mb-4">
                <label className="text-sm font-semibold text-white/70">Ã·»°Ω∂Ó</label>
                <button
                ...
                <div>
                  ...
                  {amount && parseFloat(amount) > 0 && (
                    <div className="mt-1 text-green-400 font-semibold">
                       µº µΩ’À: {(parseFloat(amount) * 0.92).toFixed(6)} RWA
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <div className="text-white/40 text-sm">‘›Œﬁ RWA  ’“Ê</div>
          </div>
        )}
      </div>
