...
import { ExternalLink, X, ChevronDown, Filter } from 'lucide-react'
...
export function FundActivityCard() {
  ...
  const fetchActivities = useCallback(
    async (fromBlock: bigint, limit: number | null) => {
      ...
      // 本地链(Hardhat 等) 用「全量 getLogs + 解析时按 user 过滤」更稳，避免节点对 args 过滤返回空
      ...
      if (isLocalChain) {
        try {
          ...
          if (DEBUG) console.log('[FundActivity] 本地链 getLogs 原始条数:', stakingLogs.length, 'fromBlock:', fromBlock.toString(), 'stakingAddress:', stakingAddress)
        } catch (e) {
          stakingLogs = []
          if (DEBUG) console.warn('[FundActivity] 本地链 getLogs 失败:', e)
        }
      } else {
        try {
      ...
      if (DEBUG) console.log('[FundActivity] 解析后 stakingRows:', stakingRows.length, 'burnRows:', burnRows.length, 'merged:', out.length, 'user:', normalizedUser.slice(0, 10) + '...')
      return out
    },
    ...
    currentBlockPromise.then((currentBlock) => {
      ...
      fetchStakesFromApi(address, locale, 5)
        ...
        .catch((err) => {
          ...
          setLoading(false)
          fetchStakesFromApi(address!, locale, 50).then((apiRows) => setRows(apiRows))
        })
    }).catch(() => {
      setLoading(false)
    })
    ...
    >
          ...
          <table className="w-full min-w-[500px]">
            ...
            <tbody>
              {rows.slice(0, 5).map((row, i) => (
                ...
                >
                  <td className="sticky start-0 bg-inherit py-3 pe-4 font-mono text-[12px] text-[#64748b]">
                    {row.time}
                  </td>
                  <td className="py-3 pe-4">
                    <TypePill label={t(`fundActivity.summary${row.summaryType.charAt(0).toUpperCase() + row.summaryType.slice(1)}`)} variant="neutral" />
                  </td>
                  <td className="py-3 pe-4">
      ...
      {showModal && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          >
            {/* 标题栏：细线点缀 + 关闭 */}
            <div className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                <div className="h-8 w-0.5 rounded-full bg-[#00f5d4]/60" aria-hidden />
              ...
              <button
                ...
                className="rounded-full p-2 text-[#64748b] hover:text-[#f1f5f9] hover:bg-white/[0.06] transition-colors duration-200"
                aria-label="Close"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            <div className="flex-1 min-h-0 overflow-auto overscroll-contain">
              {modalLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-2">
                  <p className="text-[14px] text-[#64748b]">{t('fundActivity.noRecords')}</p>
                </div>
              ) : (() => {
                const filteredRows = modalCategory === '' ? modalRows : modalRows.filter((r) => r.typeKey === modalCategory)
                return filteredRows.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 gap-2">
                    <p className="text-[14px] text-[#64748b]">{t('fundActivity.noRecordsForType')}</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[520px]">
                      <thead>
                        <tr className="border-b border-white/[0.06] bg-[#0a0a10]/80">
                      ...
                      <tbody>
                        {filteredRows.map((row, i) => (
                          <tr
                            ...
                            className="border-b border-white/[0.04] transition-colors duration-150 hover:bg-[#00f5d4]/[0.03]"
                          >
                            <td className="sticky left-0 z-10 bg-inherit px-4 sm:px-6 py-3.5 font-mono text-[12px] text-[#94a3b8]">
                              {row.time}
                            </td>
