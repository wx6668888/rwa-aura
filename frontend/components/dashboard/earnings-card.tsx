...
export function EarningsCard() {
  ...
  useEffect(() => {
    ...
    const calculateEarnings = () => {
      ...
      // - if API exists: API total (flex+locked) - locked = flex remaining
      // - if API missing: use chain totals directly (so dynamic yield doesn't disappear)
      const apiTotalUSDT = apiData ? parseFloat(apiData.usdtStaked) / 1e18 : totalFlexUSDT + totalLockedUSDT
      const apiTotalRWA = apiData ? parseFloat(apiData.rwaStaked) / 1e18 : totalFlexRWA + totalLockedRWA
      ...
      if (activeStakes.length === 0) {
        ...
        if (rwaStakeInfo?.totalStakedRWA && parseFloat(rwaStakeInfo.totalStakedRWA) > 0) {
          // Contract hooks return human-readable token amount (e.g. "2000"),
          // but downstream earnings logic expects wei-like integer string.
          const rwaWei = BigInt(Math.floor(parseFloat(rwaStakeInfo.totalStakedRWA) * 1e18)).toString()
          fallbackStakes.push({
        ...
        if (userStakeInfo?.totalStaked && parseFloat(userStakeInfo.totalStaked) > 0) {
          // Same unit normalization for USDT fallback stake.
          const usdtWei = BigInt(Math.floor(parseFloat(userStakeInfo.totalStaked) * 1e18)).toString()
          fallbackStakes.push({
      ...
      // total = 自当前 UTC 日界起、各笔质押按秒预估的 RWA 增量（未写入合约 rwaPending 的部分）
      // rwaPendingNum = 链上已结算待领取；展示 = 已结算 + 本日增量（避免旧逻辑 total - pending 吞掉增量）
      setStakeEarnings(earnings)
      setTotalRwaEarning(rwaPendingNum + total)
      setUnsettledEarning(total)
    }
  ...
  /** 无质押记录时不展示：已结算/未结算、质押明细整块（链上/列表均无有效质押） */
  const hasStakeRecords =
    isConnected &&
    !stakesLoading &&
    (stakes.length > 0 || usdtStaked > 1e-9 || rwaStaked > 1e-9)
      ...
      <div className="flex h-full flex-col gap-0 md:flex-row">
        ...
        <div className="flex flex-1 flex-col">
          ...
          <div className="mt-1 flex items-baseline gap-2 min-w-0">
            <span
              className="font-mono text-[34px] sm:text-[40px] font-bold leading-none text-[#f1f5f9] transition-all whitespace-nowrap"
              style={{
            ...
            <span className="font-mono text-lg text-[#00f5d4] whitespace-nowrap">
              {t('earnings.rwaCurrency')}
            </span>
          </div>
          <p className="mt-1 text-[13px] text-[#64748b]">
            {isConnected ? `≈ $${rwaUsdtValue} USDT` : t('earnings.usdtEquiv')}
          </p>
          {isConnected && hasStakeRecords && (
            ...
            <div className="mt-4 rounded-xl border border-[#00f5d420] bg-gradient-to-br from-[#0a0a0f] to-[#0d0d14] p-4 shadow-[0_0_20px_rgba(0,245,212,0.05)]">
              <div className="mb-3 flex items-center justify-between">
                ...
                <p className="text-xs text-[#64748b]">
                  {stakeEarnings.length > 0 ? `${stakeEarnings.length} ${t('earnings.stakes') || '笔'}` : (t('earnings.fundDetails') || '资金明细')}
                </p>
              </div>
              {/* 说明：质押明细显示实时计算的预估收益 */}
                ...
                {stakeEarnings.length > 0 ? (
                  stakeEarnings.map((earning, index) => {
                    ...
                    return (
                      <div
                        key={uniqueKey}
                        className="flex flex-col gap-2 rounded-lg border border-[#ffffff08] bg-[#0d0d14] px-3 py-2.5 transition-all hover:border-[#00f5d430] hover:bg-[#13131e] sm:flex-row sm:items-start sm:justify-between sm:gap-3"
                      >
                        <div className="min-w-0 flex-1">
                          <span className="block text-sm font-semibold break-words text-[#f1f5f9]">
                            {isRWAStake
                          ...
                          <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1">
                            <span className="text-xs text-[#64748b]">{getLockPeriodLabel(earning.lockPeriod)}</span>
                            <span className="text-[10px] text-[#64748b]">• {earning.elapsedDays.toFixed(2)} 天</span>
                            <span className="font-mono text-[10px] text-[#00f5d4] break-all">
                        ...
                        <div className="min-w-0 w-full text-left sm:w-auto sm:text-right">
                          <span
                            className="block font-mono text-[13px] font-semibold tabular-nums text-[#00f5d4] transition-all break-words [overflow-wrap:anywhere] sm:break-normal"
                            style={{
                              animation: 'pulse 2s ease-in-out infinite',
                          ...
                          >
                            {earning.rwaEarning.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 })}{' '}
                            RWA
                          </span>
                          <p className="mt-0.5 break-words text-[10px] text-[#64748b] [overflow-wrap:anywhere]">
                  ...
                  // 无每笔明细时仍显示资金明细：以链上待领取为主
                  <div className="rounded-lg bg-[#0d0d14] px-3 py-2.5 border border-[#ffffff05]">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[#64748b]">{locale?.startsWith('zh') ? '已结算（链上）' : 'Settled (on-chain)'}</span>
                      <span 
                      ...
                      >
                        {(totalRwaEarning || rwaPendingNum).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 })} RWA
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="mt-3 border-t border-[#ffffff0d] pt-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="font-mono text-xs text-[#64748b] block mt-0.5">
                    ≈ ${((totalRwaEarning || rwaPendingNum) * rwaPrice).toFixed(2)} USDT
                  </span>
                </div>
              </div>
            </div>
