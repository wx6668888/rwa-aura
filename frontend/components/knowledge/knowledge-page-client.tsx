...
const KNOWLEDGE_UI_VERSION = '2026.03.20-r1'
    ...
    <div className="relative z-10 min-h-screen pb-24 pt-below-navbar-safe">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <span className="rounded-md border border-[rgba(0,245,212,0.3)] bg-[rgba(0,245,212,0.08)] px-2 py-1 text-[11px] text-[#7ef7e4]">
            知识库版本 {KNOWLEDGE_UI_VERSION}
          </span>
          <button
            type="button"
            onClick={() => window.location.reload()}
            className="rounded-md border border-[rgba(255,255,255,0.1)] px-2 py-1 text-[11px] text-[#94a3b8] transition-colors hover:border-[rgba(255,255,255,0.2)] hover:text-[#e2e8f0]"
          >
            刷新页面获取最新内容
          </button>
        </div>
        ...
        <div className="mt-4 grid gap-2">
          <button
            ...
            onClick={() => {
              ...
              setOpenArticleId('binance-builtin-browser-issues')
              setDrawerOpen(false)
              setTimeout(() => {
                const el = sectionRefs.current['security']
                if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' })
              }, 100)
            }}
            className="w-full rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] px-3 py-2 text-left text-[13px] text-[#cbd5e1] hover:border-[rgba(0,245,212,0.3)]"
          >
            最新：币安/欧易内置浏览器常见问题排查
          </button>
          <button
            ...
            onClick={() => {
              setCategoryFilter('security')
              setOpenArticleId('app-vs-system-browser-diff')
              setDrawerOpen(false)
              setTimeout(() => {
                const el = sectionRefs.current['security']
                if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' })
