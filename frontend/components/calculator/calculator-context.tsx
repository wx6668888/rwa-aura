...
interface CalculatorResults {
  ...
  /** 项目分红（仅 L4+，按“投资份额→国库收益→40%分红”估算） */
  projectDividend: number;
...
function CalculatorProviderInner({ children }: { children: React.ReactNode }) {
  ...
  const results = useMemo<CalculatorResults>(() => {
    ...
    // 项目分红（按之前口径：国库投资收益的一部分分给用户）
    // - 国库投资年化假设：4%
    // - 分红比例：国库投资收益的 40% 分给用户
    // - 资格：L4 及以上
    const levelIndex = NODE_LEVELS.findIndex((cfg) => cfg.code === nodeLevel);
    const projectDividendEligible = levelIndex >= 3; // L4+
    const treasuryAnnualReturn = 0.04;
    const dividendShare = 0.40;
    const projectDividend = projectDividendEligible
      ? investmentShares * treasuryAnnualReturn * dividendShare * (days / 365)
      : 0;
