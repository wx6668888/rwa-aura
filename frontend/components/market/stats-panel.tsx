...
export function StatsPanel() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('0xRWA...Token')
    setCopied(true)
    ...
    <div className="space-y-5 rounded-xl border border-[#ffffff0d] bg-[#0d0d1499] p-5 backdrop-blur-xl lg:sticky lg:top-24">
      ...
      <div>
        ...
        <div className="mt-3 space-y-2">
          <LinkItem label={t('market.linkPancakeswap')} />
          <LinkItem label={t('market.linkBscscan')} />
          <LinkItem label={t('market.linkAddToWallet')} />
          <LinkItem label={t('market.linkWebsite')} />
        </div>
      </div>
    </div>
  )
