...
**https://rwaprotocol.dpdns.org**

配置见根目录 `capacitor.config.ts`。

### 重要：`server.allowNavigation`

若未把业务域名列入 `server.allowNavigation`，在 WebView 里导航到 `https://rwaprotocol.dpdns.org/...` 时，**Capacitor 会交给系统浏览器打开（Chrome）**，看起来像「一打开就跳到浏览器」。当前已配置：

- `rwaprotocol.dpdns.org`
...
- **`NEXT_PUBLIC_APP_URL`**：请设为 `https://rwaprotocol.dpdns.org`（与壳加载地址一致，利于 WalletConnect / 分享链接）。
...
3. 在 [WalletConnect Cloud](https://cloud.walletconnect.com/) 项目里，如有 **Allowed redirect / Domain** 之类配置，把 **`https://rwaprotocol.dpdns.org`** 与 **`rwaprotocol://wc`** 按面板说明加入白名单（以控制台为准）。
