...
export class EventMonitor {
    ...
    private async processBlockRange(fromBlock: number, toBlock: number): Promise<void> {
        ...
        // 当前区块段没有任何合约日志时，跳过后续所有 queryFilter 请求，显著降低 RPC 压力
        if (logs.length === 0) {
            return;
        }
    ...
    private async handleStakeEvent(event: ethers.EventLog): Promise<void> {
        try {
            ...
            const existing = await query<Stake[]>(
                'SELECT stake_id FROM stake_events WHERE stake_id = ? AND tx_hash = ?',
                [stakeId.toString(), txHash]
            );
            ...
            await transaction(async (connection) => {
                ...
                // In concurrent workers, INSERT IGNORE can be ignored for duplicates (affectedRows=0).
                // Only the worker that successfully inserts should write snapshots / rewards.
                stakeInserted = (insertResult?.affectedRows ?? 0) > 0;
                if (!stakeInserted) return;
            ...
            if (!stakeInserted) {
                logger.warn(`StakeEvent insert ignored (concurrent dup): stakeId=${stakeId}, tx=${txHash}`);
                return;
            }
    ...
    private async handleRWAStakeEvent(event: ethers.EventLog): Promise<void> {
        try {
            ...
            await transaction(async (connection) => {
                ...
                // Prevent duplicate snapshots in concurrent workers.
                stakeInserted = (insertResult?.affectedRows ?? 0) > 0;
                if (!stakeInserted) return;
            ...
            if (!stakeInserted) {
                logger.warn(`RWAStakeEvent insert ignored (concurrent dup): stakeId=${stakeId}, tx=${txHash}`);
                return;
            }
