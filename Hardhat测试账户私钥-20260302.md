# 🔑 Hardhat 测试账户私钥 - 2026年3月2日

**网络**: Hardhat Local (Chain ID: 31337)  
**每个账户余额**: 10000 ETH

---

## 📋 测试账户列表

### Account #0 (推荐使用)
```
地址: 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
私钥: 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80
余额: 10000 ETH
```

---

### Account #1
```
地址: 0x70997970C51812dc3A010C7d01b50e0d17dc79C8
私钥: 0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d
余额: 10000 ETH
```

---

### Account #2
```
地址: 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC
私钥: 0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a
余额: 10000 ETH
```

---

### Account #3
```
地址: 0x90F79bf6EB2c4f870365E785982E1f101E93b906
私钥: 0x7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6
余额: 10000 ETH
```

---

### Account #4
```
地址: 0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65
私钥: 0x47e179ec197488593b187f80a00eb0da91f1b9d0b13f8733639f19c30a34926a
余额: 10000 ETH
```

---

### Account #5
```
地址: 0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc
私钥: 0x8b3a350cf5c34c9194ca85829a2df0ec3153be0318b5e2d3348e872092edffba
余额: 10000 ETH
```

---

### Account #6
```
地址: 0x976EA74026E726554dB657fA54763abd0C3a0aa9
私钥: 0x92db14e403b83dfe3e2330be8552f375b6735b9c5b462a92e077356f1c87b0a
余额: 10000 ETH
```

---

### Account #7
```
地址: 0x14dC79964da2C08b23698B3D3cc7Ca32193d9955
私钥: 0x4bbbf85ce3377467afe5d46f804f221813b2bb87f24d81f60f1fcdbf7cbf4356
余额: 10000 ETH
```

---

### Account #8
```
地址: 0x23618e81E3f5cdF7f54C3d65f7FBc0aBf5B21E8
私钥: 0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97
余额: 10000 ETH
```

---

### Account #9
```
地址: 0xa0Ee7A142d267C1f36714E4a8F75612F20a79720
私钥: 0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff04d545c
余额: 10000 ETH
```

---

## 🚀 快速导入（推荐 Account #0）

**复制这个私钥到 MetaMask**:
```
0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80
```

**导入步骤**:
1. MetaMask → 账户 → 导入账户
2. 粘贴上面的私钥
3. 确认导入

---

## 📝 注意事项

1. **这些是测试账户，仅用于开发**
2. **不要在主网使用这些私钥**
3. **每次 Hardhat 节点启动，这些账户都是相同的**
4. **每个账户都有 10000 ETH（测试币）**

---

## ✅ 验证导入

导入后检查：
- [ ] 账户地址: `0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266`
- [ ] 余额: 10000 ETH
- [ ] 网络: Hardhat Local (Chain ID: 31337)

---

**推荐使用 Account #0 进行测试！**
