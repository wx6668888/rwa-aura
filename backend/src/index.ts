...
import { TronTopupMonitorService } from './services/TronTopupMonitorService';
...
class BackendService {
    ...
    private tronTopupMonitorService!: TronTopupMonitorService;
    ...
    private initializeServices(): void {
        ...
        // Tron Topup Monitor (TRC20 USDT incoming detection)
        this.tronTopupMonitorService = new TronTopupMonitorService({
            tronGridBaseUrl: process.env.TRONGRID_BASE_URL || 'https://api.trongrid.io',
            usdtTrc20Contract: process.env.TRON_USDT_TRON_CONTRACT || 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t',
            apiKey: process.env.TRONGRID_API_KEY || process.env.TRON_PRO_API_KEY,
            pollIntervalMs: Math.max(
                5000,
                (process.env.TRON_TOPUP_POLL_SECONDS ? Number(process.env.TRON_TOPUP_POLL_SECONDS) : 20) * 1000
            ),
            maxOrdersPerPoll: Math.max(
                1,
                Number(process.env.TRON_TOPUP_MAX_ORDERS_PER_POLL || '10')
            ),
            requestTimeoutMs: Math.max(
                2000,
                Number(process.env.TRON_TOPUP_REQUEST_TIMEOUT_MS || '8000')
            ),
            confirmationBlocks: Math.max(
                1,
                Number(process.env.TRON_TOPUP_CONFIRMATION_BLOCKS || '12')
            ),
        });
    ...
    async start(): Promise<void> {
        try {
            ...
            // Start tron topup monitor
            logger.info('Starting tron topup monitor service...');
            this.tronTopupMonitorService.start();
            logger.info('✅ Tron topup monitor started');
    ...
    async stop(): Promise<void> {
        ...
        try {
            ...
            // Stop tron topup monitor
            this.tronTopupMonitorService.stop();
