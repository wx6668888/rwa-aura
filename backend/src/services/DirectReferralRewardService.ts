...
export class DirectReferralRewardService {
    ...
    async recordReferralReward(
        ...
        try {
            ...
            // 计算奖励：统一以 USDT 存入 reward_amount。规则：不锁仓不触发；RWA 需 ×0.85 折算成 USDT
            const stakeAmountDecimal = new BigNumber(stakeAmount).dividedBy(1e18);
            const rewardBps = stakeAmountDecimal.multipliedBy(rewardRate).dividedBy(10000);
            const rewardAmount = stakeType === 'RWA'
                ? rewardBps.multipliedBy(0.85)
                : rewardBps;
