...
app.get('/api/rwa-nonce/:address', async (req, res) => {
  try {
    ...
    const rwa = new ethers.Contract(RWA_TOKEN, rwaAbi, provider);
...
app.get('/api/usdt-nonce/:address', async (req, res) => {
  try {
    ...
    const usdt = new ethers.Contract(USDT_TOKEN, usdtAbi, provider);
...
// 初始化 UserStatsService
const userStatsService = new UserStatsService(db);

// 启动时更新一次所有用户统计
console.log('[UserStats] 初始化用户统计数据...');
userStatsService.updateAllUsers();

// 每10分钟更新一次
setInterval(() => {
  console.log('[UserStats] 定时更新用户统计数据...');
  userStatsService.updateAllUsers();
}, 10 * 60 * 1000);

// API: 立即更新指定用户的统计数据
app.post('/api/user-stats/update/:address', (req, res) => {
  try {
    const { address } = req.params;
    userStatsService.updateUserStats(address);
